...
contract ElectricEthereum { 
    ...
    function OwnerManualTurnOffElectricity() public onlyOwner {
        ...
        for(uint currentLED; currentLED < 4; currentLED++) {
            if(block.timestamp > LED[currentLED].ExpirationTimeUNIX && LED[currentLED].Voltage == 1){
                LED[currentLED].Voltage  = 0;
                LED[currentLED].ExpirationTimeUNIX = 0;
                LED[currentLED].LatestBuyer = 0x0000000000000000000000000000000000000000;
             }
